...
import { GitGraph, Search, ChevronRight, ChevronDown, ChevronUp } from "lucide-react";
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
        <p className="text-xs text-muted-foreground shrink-0">{filtered.length} trace{filtered.length !== 1 ? "s" : ""} recorded</p>
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full lg:w-auto">
          <div className="relative flex-1 sm:w-[240px]">
            <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-muted-foreground" />
            <Input placeholder="Filter by question or session…" value={filter} onChange={(e) => setFilter(e.target.value)} className="pl-8 h-8 text-xs bg-card border-border w-full" />
          </div>
          <Select value={sort} onValueChange={(v) => setSort(v as SortOrder)}>
            <SelectTrigger className="h-8 w-full sm:min-w-[140px] text-xs font-semibold"><SelectValue /></SelectTrigger>
            <SelectContent>
                ...
                <CardContent className="p-4">
                  ...
                  {t.sql && (
                    <div className="mt-2 relative">
                      <button
                        onClick={() => setExpandedSql(expandedSql === `${t.session_id}-${t.ts}` ? null : `${t.session_id}-${t.ts}`)}
                        className="w-full bg-muted border border-border rounded px-2.5 py-1.5 font-mono text-[10px] text-muted-foreground text-left hover:bg-muted/80 transition-colors flex items-center justify-between gap-2"
                      >
                        <span className={expandedSql === `${t.session_id}-${t.ts}` ? "whitespace-pre-wrap" : "truncate"}>
                          {t.sql}
                        </span>
                        {expandedSql === `${t.session_id}-${t.ts}` ? (
