...
import { fetchSessions, fetchConnections } from "@services/api";
...
export default function ChatOverviewPage() {
  ...
  const [activeDialect, setActiveDialect] = useState<string>("postgresql");
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [sessRes, connRes] = await Promise.all([fetchSessions(200), fetchConnections()]);
      if (sessRes) setSessions(sessRes.sessions || []);
      if (connRes) {
        // Intentionally not setting activeAlias to allow user to connect individually
      }
  ...
  const handleNewChat = () => {
    ...
    router.push(`/connection/chat/${newId}`);
  };
    ...
    <div className="w-full max-w-6xl mx-auto space-y-5">
      ...
      {loading ? (
        ...
        <div className="border border-dashed rounded-xl p-14 text-center">
          ...
          {!search && !filterErr && (
            <Button size="sm" className="h-8 text-xs gap-1.5" onClick={handleNewChat} disabled={!activeAlias}>
              <Plus className="h-3.5 w-3.5" /> New Chat
            </Button>
          )}
        </div>
            ...
            {filtered.map((s, idx) => (
              <motion.div key={s.session_id}
                ...
                <Card
                  ...
                  onClick={() => router.push(`/connection/chat/${s.session_id}`)}>
