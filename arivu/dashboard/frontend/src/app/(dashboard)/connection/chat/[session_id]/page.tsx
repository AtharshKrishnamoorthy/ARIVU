...
import { chatDB, fetchConnections, setActiveConnection, fetchSessionDetail, saveRLHFSignal, visualizeData, exportData, fetchSuggestions, fetchDashboards, addDashboardWidget, type Dashboard } from "@services/api";
...
function MessageCard({
  ...
  return (
    <div className={`flex gap-3 ${isUser ? "flex-row-reverse" : "flex-row"}`}>
      ...
      {/* Card */}
      <div className={`max-w-[78%] rounded-2xl overflow-hidden border
        ${isUser
                ...
                <Tabs defaultValue="response" className="w-full mt-2">
                  <div className="flex items-center gap-2 mb-3">
                    <TabsList className="grid flex-1 grid-cols-3 bg-muted/50 p-1 rounded-xl h-9">
                      <TabsTrigger value="response" className="text-xs rounded-lg data-[state=active]:bg-background data-[state=active]:shadow-sm">Response</TabsTrigger>
                      <TabsTrigger
                        value="visualize"
                      ...
                      >
                        <BarChart3 className="w-3 h-3 mr-1.5" /> Visualize
                      </TabsTrigger>
                      <TabsTrigger value="table" className="text-xs rounded-lg data-[state=active]:bg-background data-[state=active]:shadow-sm">
                        <Table2 className="w-3 h-3 mr-1.5" /> Data Table
                      </TabsTrigger>
                    </TabsList>
                    ...
                    {msg.c1_response && (
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button variant="secondary" size="sm" className="h-9 px-3 text-xs gap-1.5 shrink-0 border-border/60 bg-background/95 hover:bg-background">
                  ...
                  <TabsContent value="visualize" className="mt-0 outline-none min-h-[150px]">
                    ...
                    {msg.c1_response && C1Component && (
                      <div className="c1-viz-wrapper rounded-xl border border-border overflow-hidden bg-background">
                        {ThemeProvider ? (
                          <ThemeProvider theme={typeof document !== "undefined" && document.documentElement.classList.contains("dark") ? "dark" : "light"}>
                            <C1Component c1Response={msg.c1_response} />
                          </ThemeProvider>
                        ) : (
                          <C1Component c1Response={msg.c1_response} />
                        )}
...
export default function ChatSessionPage({ params }: { params: Promise<{ session_id: string }> }) {
  ...
  /* Helper: Get localStorage key for this session's connection */
  const getSessionStorageKey = () => `arivu_session_${sessionId}_activeAlias`;

  /* Helper: Load activeAlias from localStorage */
  const loadSavedAlias = useCallback(() => {
  ...
  /* Helper: Save activeAlias to localStorage */
  const saveAliasToStorage = useCallback((alias: string) => {
    if (typeof window !== "undefined") {
      if (alias) {
        localStorage.setItem(getSessionStorageKey(), alias);
      } else {
  ...
  const loadInitial = useCallback(async () => {
    ...
    try {
      const [connRes, sessRes, suggRes, dashRes] = await Promise.all([
        fetchConnections(),
      ...
      if (connRes) {
        ...
        // Restore activeAlias from localStorage if it exists
        const savedAlias = loadSavedAlias();
        if (savedAlias && connRes.connections?.some((c: any) => c.alias === savedAlias)) {
          setActiveAlias(savedAlias);
          const conn = connRes.connections.find((c: any) => c.alias === savedAlias);
          if (conn) setActiveDialect(conn.dialect || "postgresql");
        }
      }
      ...
      if (sessRes?.history?.length) {
        ...
        // If there is history, try fetching follow-ups for the last question
  ...
  /* Persist activeAlias to localStorage whenever it changes */
  useEffect(() => {
    saveAliasToStorage(activeAlias);
  }, [activeAlias, saveAliasToStorage]);

  /* Restore connection on page visibility change (tab switch, window focus) */
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.visibilityState === "visible" && activeAlias) {
        // Page became visible; optionally re-establish connection if needed
        setActiveConnection(activeAlias).catch(() => {
          // Connection re-establishment failed; user can manually reconnect if needed
        });
      }
    };
  ...
  const handleSwitch = async (alias: string) => {
    if (!alias || alias === "none") return;
    setSwitching(true);
    try {
      await setActiveConnection(alias);
      setActiveAlias(alias); // Also saves to localStorage via useEffect
      const conn = connections.find((c: any) => c.alias === alias);
      if (conn) setActiveDialect(conn.dialect || "postgresql");
      toast.success(`Connected to ${alias}`);
      
      // Refresh suggestions for new connection
      fetchSuggestions().then(res => {
        if (res?.suggestions) setSuggestions(res.suggestions);
      }).catch(() => {});
      setFollowUpSuggestions([]); // Clear follow-ups on switch
    } catch (e: any) {
      // Clear the stored alias if connection fails
      saveAliasToStorage("");
  ...
  const handleSend = useCallback(async (overrideMsg?: string) => {
    ...
    if (!text || loading || switching) return;
    if (!activeAlias) { toast.error("Select a connection first."); return; }
  ...
  }, [input, loading, switching, activeAlias, sessionId, addMsg]);
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 -mx-4 sm:-mx-6 -mt-0">
      {/* ── Top bar ── */}
      <div className="flex items-center gap-3 px-4 sm:px-6 py-2.5 border-b border-border bg-background/80 backdrop-blur-sm shrink-0">
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          <Select value={activeAlias || "none"} onValueChange={handleSwitch}
            disabled={switching || connections.length === 0}>
            <SelectTrigger className="h-8 w-44 text-xs font-medium border-0 bg-muted/50">
              <SelectValue placeholder="Select connection" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="none" disabled>— Select connection —</SelectItem>
              {connections.map((c: any) => (
                <SelectItem key={c.alias} value={c.alias}>
                  <div className="flex items-center gap-2">
                    <img src={DIALECT_LOGOS[c.dialect] || ""} alt="" className="w-3.5 h-3.5 object-contain" />
                    {c.alias}
                  </div>
                </SelectItem>
              ))}
          ...
          {switching ? (
            ...
            <Badge variant="outline" className="h-5 px-2 text-[10px] gap-1 shrink-0">
      ...
      <div className="relative z-10 shrink-0 px-4 sm:px-8 pb-6 sm:pb-8 pt-2 max-w-4xl mx-auto w-full">
        <div className={`relative flex items-end gap-2 bg-background shadow-[0_4px_40px_rgba(0,0,0,0.08)] dark:shadow-[0_4px_40px_rgba(0,0,0,0.3)] border border-border/80 rounded-[28px] px-3 py-2 transition-all
          ...
          <textarea
            ...
            disabled={!isConnected || loading || switching}
            rows={1}
          ...
          <button
            onClick={() => handleSend()}
            disabled={!input.trim() || loading || !isConnected || switching}
